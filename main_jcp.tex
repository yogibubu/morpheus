...
\texttt{SONIC} (Symmetry-Oriented Non-redundant Internal Coordinates). A
frozen SONIC contract contains the non-redundant symmetry-adapted GIC basis,
the corresponding Cartesian reference space, symmetry labels, Wilson
$\mathbf{B}$ rows, and provenance. \texttt{MORPHEUS} is the refinement layer
built on those definitions: it selects experimentally supported directions,
...
internal-coordinate (PIC) set and its Wilson matrix. The
PIC set contains
...
documented in the companion SONIC methodological manuscript;
...
symmetry-operation residual margins, the SALC gauge policy, Wilson
...
\begin{table*}[t]
\caption{Camphor final-validation controls. RMSD and maximum atomic shifts
are computed after alignment to the accepted no-Kraitchman BDPCS3-predicate
reference geometry.}
...
\begin{table*}[t]
\caption{Glycine II constraint-source comparison. Both calculations used GIC
coordinates, moments of inertia as observables, and the same 10 corrected
Gly-IIn rotational data sets. Rotational residuals are reported over the
diagnostic $A$, $B$, and $C$ constants in MHz. The last column is the
mean-square convention used for comparison with the published glycine
refinement, not a second RMS value.}
...
\begin{table}[t]
\caption{Effect of weighted predicates on the glycine reduced-dimensionality
fits. The predicate runs use the same experimental data and hard functional
constraints as the corresponding benchmark runs, plus initial-geometry
predicates with $\sigma_R=0.003$~\AA{}, $\sigma_A=0.3^\circ$, and
$\sigma_D=0.5^\circ$. RMS values are complete rotational residuals in MHz.}
...
\caption{Testosterone large-molecule scale demonstration. The 49-atom
fused-ring molecule tests the complete path from a QC reference geometry and
vibrational corrections to automated non-redundant coordinate generation and
reduced structural models.}
\label{fig:testosterone-scale}
\end{figure}

\begin{table*}[t]
\caption{Testosterone rotational-constant residuals relative to the corrected
experimental equilibrium constants derived from the published $B_0$ and
$\Delta B_\mathrm{vib}$ values. Residuals are reported in MHz.}
...
article and its supplementary material. Machine-readable benchmark inputs,
outputs, numerical audit trails, and the code version used for the reported
calculations are available from the corresponding author upon reasonable
request. The companion SONIC methodological preprint and its public identifier will be
cited in the final submission after completion of arXiv processing.
