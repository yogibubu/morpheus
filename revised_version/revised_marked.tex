...
\begin{abstract}
\rev{Semiexperimental equilibrium structures combine high-resolution rotational spectroscopy with computed vibrational and electronic corrections. Although the numerical least-squares solution is well established, constructing and validating the inverse model that connects observables to structural parameters often still requires expert choices of coordinates, symmetry, constraints, predicates, and parameter classes. We introduce \texttt{MORPHEUS} (Model Optimization and Refinement from Perceived Hierarchy, Equivalence, Uncertainty, and Symmetry), a framework that makes this formulation reproducible, auditable, and constrained by topology, symmetry, rank, isotope coverage, and explicit external reference information. Starting from Cartesian geometry, the workflow builds non-redundant internal coordinates and a refinement model before fitting. It offers two equivalent active spaces: symmetry-adapted generalized internal coordinates assembled from homogeneous primitive families and symmetry-adapted Cartesian displacements with translations and rotations removed. The chemical interface remains internal-coordinate based in both representations. Tests spanning acyclic, cyclic, planar, fused, bridged, and flexible molecules reproduce effective dimensions, residuals, structural parameters, and propagated uncertainties without manual coordinate-model construction. In camphor, the accepted no-Kraitchman model with BDPCS3-derived predicates yields sub-milliangstrom bond corrections and remains stable under coordinate-basis changes, robust losses, predicate-width scans, and multistarts, whereas numerically competitive alternatives are rejected because they distort the geometry. A 49-atom testosterone case further demonstrates that accurate equilibrium geometries, accelerated vibrational corrections, and generated class-constrained refinement models can be chained when isotope-rich data are unavailable. MORPHEUS therefore turns expert-dependent model formulation into an explicit, rank-tested, and reusable stage of semiexperimental structure determination.}
...
refinement model from the realized geometry. The symmetry-Cartesian route uses
...
\rev{The complete sequence is summarized in Figure~\ref{fig:glycine-algorithmic-flow}.}
...
\rev{The testosterone demonstration extends this logic to the largest system considered here.} It does not claim a complete
...
diagnostics are documented in the Supporting Information. The standalone
\rev{MORPHEUS 0.1.0rc8 source, runnable examples, benchmark inputs and outputs,
and final-validation data are distributed with that release.} The intramolecular primitive
...
\texttt{SMITH}/\texttt{SONIC} methodological manuscript is publicly available
as arXiv:2607.16550.\cite{Barone2026SONIC} This does not affect the present
