...
coordinate contracts. The contract contains the non-redundant,
symmetry-adapted GIC basis, the symmetry-adapted Cartesian reference spaces,
symmetry labels, provenance, and the analytic Wilson $\mathbf{B}$ rows needed
by downstream modules. \texttt{MORPHEUS} is the refinement layer built on those
...
tests the stability of the accepted model. The molecular-state perception used
by \texttt{SMITH} follows the continuous molecular perception and synthon
descriptors introduced in earlier work.\cite{Lazzari2020Proxima,Lazzari2024SynthonPCS}
...
within-family redundancies, form their symmetry-adapted combinations, and
evaluate the associated Wilson $\mathbf{B}$ matrices are implemented in
\texttt{SMITH} as \texttt{SONIC} coordinate contracts. They are documented in a
companion methodological manuscript, currently in preparation for publication;
...
reusable molecular-state, coordinate, and derivative services. \texttt{SMITH}
builds or consumes the frozen molecular state from Cartesian atoms and nuclear
charges, topology, point-group operations, perception descriptors, and
synthons, and stores the resulting \texttt{SONIC} coordinate contract,
...
coordinate model and updates only the derivative information needed for the
current step.
...
available from the author upon request. The intramolecular coordinate
definitions and Wilson $\mathbf{B}$ matrices used in the present work were
generated as \texttt{SONIC} contracts by \texttt{SMITH}; the companion
\texttt{SMITH}/\texttt{SONIC} methodological manuscript is in preparation for
publication and can be supplied as confidential supplementary material for
