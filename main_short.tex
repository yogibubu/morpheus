...
In this implementation, the Nonredundant Equivariant Orthogonalizer (NEO) is
the coordinate engine: it builds non-redundant, symmetry-adapted GIC and
Cartesian bases from the molecular graph. \texttt{MORPHEUS} is the refinement
layer built on those bases: it selects experimentally supported directions,
applies constraints, classes, and predicates, solves the least-squares problem,
and tests the stability of the accepted model.
The molecular-state information consumed by NEO is supplied by ORACLE, a new
implementation of the continuous molecular perception and synthon descriptors
introduced in earlier work.\cite{Lazzari2020Proxima,Lazzari2024SynthonPCS}
Thus ORACLE fixes atom perception, topology, symmetry operations, and chemical
descriptors once; NEO freezes the resulting coordinate contract; and MORPHEUS
uses that contract without regenerating the model during refinement.
...
topology-perception implementation feeds it. In the calculations reported here,
that molecular-state layer is ORACLE, which follows the continuous-perception
and synthon strategy cited above. The present
...
reusable molecular-state, coordinate, and derivative services. ORACLE builds
the molecular state from Cartesian atoms and nuclear charges, assigns topology,
point-group operations, perception descriptors, and synthons; NEO constructs
and stores the frozen coordinate definition, including the symmetry-operation
residual margins and SALC gauge policy introduced in the companion NEO work.
A separate B-matrix service evaluates that same definition for any compatible
Cartesian geometry. During refinement, the engine reuses the stored coordinate
model and updates only the derivative information needed for the current step.
...
The intramolecular NEO coordinate generator therefore follows the
...
available from the author upon request. The intramolecular coordinate-generation
path used in the present work has been exercised over the benchmark set reported
here. The public NEO/ORACLE release is planned with the companion
methodological paper, where intermolecular coordinates and additional
special-coordinate classes will be treated explicitly. This does not affect the
present applications, because the final-validation workflow replays accepted
models in
