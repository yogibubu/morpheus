...
\begin{abstract}
Semiexperimental equilibrium structures combine high-resolution rotational spectroscopy with computed vibrational and electronic corrections, but the inverse model connecting observables to structural parameters is still commonly formulated by expert intervention. This paper introduces MORPHEUS, which makes model construction a reproducible, symmetry- and rank-audited step. Molecular topology defines a redundant primitive space; symmetry selects admissible directions; isotopic coverage identifies experimentally supported combinations; and electronic-structure information enters only through explicit constraints, predicates, or parameter classes. The resulting models can be solved in either non-redundant symmetry-adapted internal coordinates or symmetry-adapted Cartesian displacements, while retaining the same chemically readable primitive relations. Validation on acyclic, cyclic, planar, fused, bridged, and flexible systems reproduces ranks, residuals, structural parameters, and propagated uncertainties without molecule-specific coordinate editing. The norbornane-derived series separates numerical fit quality from chemical validity: the accepted camphor model yields sub-milliangstrom bond corrections and remains stable under coordinate-basis replay, robust losses, predicate-width scans, and multistarts, whereas models based on unsupported hydrogen directions or tight substitution-coordinate predicates can retain attractive rotational residuals while distorting the geometry. A 49-atom testosterone application further shows that accurate quantum-chemical geometries, accelerated vibrational corrections, and automatically generated topology-constrained refinement models can be chained when isotope-rich data sets are unavailable. MORPHEUS therefore shifts semiexperimental refinement from expert-dependent parameter selection to an explicit and auditable construction of the statistical molecular model.
\end{abstract}
...
In this implementation, Symmetry-Oriented Non-redundant Internal Coordinates
(SONICs) provide the non-redundant internal-coordinate family. A frozen SONIC
contract contains the non-redundant symmetry-adapted generalized
internal-coordinate (GIC) basis,
the corresponding Cartesian reference space, symmetry labels, Wilson
$\mathbf{B}$ rows, and provenance. MORPHEUS (Model Optimization and Refinement
from Perceived Hierarchy, Equivalence, Uncertainty, and Symmetry) is the
refinement layer built on those definitions: it selects experimentally supported directions,
applies constraints, classes, and predicates, solves the least-squares
problem, and tests the stability of the accepted model. Topology, symmetry
operations, chemical descriptors, coordinate definitions, and derivative
conventions are therefore fixed once and are not regenerated during
refinement. The parameter classes introduced below are statistical equality
or predicate relations for the semiexperimental inverse problem.
...
Reference geometries from QC computations or from high-fidelity ring-fragment
libraries\cite{Lazzari2025LCB25} enter only as chemically explicit constraints,
...
topology. Bonded pairs and ring connectivity define the redundant primitive
internal-coordinate (PIC) set and its Wilson matrix. The PIC set contains
stretches, valence angles, torsions, out-of-plane deformations,
...
The coordinate-generation stage constructs the local primitive families,
evaluates their Wilson $\mathbf{B}$ matrix, removes within-family
redundancies, forms the symmetry-adapted combinations, and stores the
transformed rows as \texttt{SONIC} coordinate contracts. Their construction is
documented in the companion SONIC methodological manuscript;\cite{Barone2026SONIC}
...
diagnostics.
The resulting SONIC basis is inspectable at the level used by the refinement,
not only through a matrix stored internally.  Each named row carries its
human-readable primitive expansion, family, symmetry label, unit, protection
and provenance fields, analytic Wilson row, and an optional $-h,0,+h$
Cartesian trajectory.  A refined, linked, or constrained variable can
therefore be traced back to both its chemical definition and its molecular
motion without reconstructing the coordinate from solver output
(Figure~\ref{fig:sonic-inspectability}).

\begin{figure*}[htbp]
\centering
\includegraphics[width=\textwidth]{figures/sonic_inspectability.pdf}
\caption{Inspectable SONIC variables at the MORPHEUS model boundary. Each row
links a primitive expansion and units to its Cartesian fingerprint and model
role. Arrows are enlarged for print and do not represent a finite refinement
step.}
\label{fig:sonic-inspectability}
\end{figure*}
...
definition and requested correction to a rank-controlled geometry-realization
service, whose minimum-norm or nonlinear procedure returns the achieved Cartesian
...
reusable molecular-state, coordinate, and derivative services. Molecular
perception freezes Cartesian atoms and nuclear charges, topology, point-group
operations, descriptors, and atom permutations. The resulting
\texttt{SONIC} coordinate contract stores family labels,
symmetry-operation residual margins, the symmetry-adapted linear-combination
(SALC) gauge policy, Wilson
$\mathbf{B}$ rows, and provenance. During refinement, \texttt{MORPHEUS}
reuses this stored coordinate model and updates only the derivative
...
The default coordinate frame places the weighted center at the origin and uses
...
uses the same local-chart atlas: chart rebasing transports both the
...
The standard input is either a versioned semiexperimental job file, with a
backward-compatible reader for legacy records, or an enriched \texttt{xyzin}
...
masses, and correction conventions. Versioned upstream rovibrational
corrections can also be promoted into the same container before they are used
by \texttt{MORPHEUS}. Their calculation and the transformation of higher
derivatives belong to the upstream vibrational-analysis layer; for the present
refinement they are input provenance, not part of the MORPHEUS model.
...
versioned final-validation audit and can replay the same
...
to primitive-coordinate relations. Kisiel's \texttt{STRFIT} program in PROSPE
(Programs for Rotational Spectroscopy)\cite{KisielPROSPE} remains an important reference point for
rotational-structure fitting, but the structural model is again supplied by
the user.\cite{Kanzow2025PCCP}
The intramolecular \texttt{SONIC} coordinate generator therefore follows the
...
\begin{table*}
\caption{Operational comparison of structure-refinement routes, emphasizing
model formulation rather than fit quality.}
...
Symmetry & In model & User supplied & Perceived and audited \\
...
hierarchical family of Pisa composite schemes (PCSs),\cite{pcs1234_2024,Lazzari2025LCB25}
using the Gaussian suite of programs.\cite{g16}

In most cases, vibrational corrections and constraint targets were taken from
the double-hybrid DPCS3 member of the family and its bond-corrected BDPCS3 enhancement,
...
The only exception is conformer I of glycine, for which the best available
reference structure was used: PCS4, based on explicitly correlated coupled-cluster
singles and doubles with perturbative triples [CCSD(T)-F12] calculations close
to the complete-basis-set limit, with an additive core--valence correction.
Outside this role as reference information, the electronic-structure
...
Unless otherwise stated, rotational residuals are reported as
...
\caption{Molecular structures used to test automated model construction.
Connectivity diagrams are used for the acyclic, planar, aromatic, glycine, and
anhydride benchmarks; three-dimensional views show the bridged norbornane
series. Atom numbering is given in the Supporting Information.}
...
\caption{Model-construction audit for the representative refinements. The left
panel gives generated coordinates, the symmetry-active subset, and final fit
variables; the right panel gives generated constraints or frozen primitives.
All cases use global defaults and no molecule-specific coordinate edits.}
...
while the legacy-compatibility table shows that imported legacy
molecular-structure-refinement program formats (MSR and MSROT) are reproduced
without changing their active-coordinate content.
...
\begin{table*}
\caption{Reproducible model decisions removed from manual setup. The final
column includes the number of molecule-specific user edits.}
...
substitutions, and the tabulated corrections. The workflow then perceives the
molecular topology, constructs the primitive/B source and non-redundant SONIC
...
In the notation below, \texttt{OOP} denotes a signed out-of-plane primitive.

\begin{table*}[t]
\caption{Representative primitive-coordinate relations generated for glycine
constraints and ring coordinates.}
...
\begin{table*}[t]
\caption{Auxiliary-relation tests for phthalic anhydride. Errors are maximum
absolute deviations from the tabulated $r_e^\mathrm{SE}$ bond lengths or
angles.}
...
Table~\ref{tab:phthalic-predicates} compares the unregularized, frozen-frame,
and predicate models.
...
$AB$ pair minimizes the complete diagnostic residual; the omitted component is
an external check.}
...
\caption{Nitrobenzene substitution-coordinate diagnostic. The upper panel
gives absolute substitution and fitted coordinates; the lower panel gives
signed differences.}
...
\caption{Glycine I fitted bond lengths and valence angles (upper panels) and
signed semiexperimental-minus-quantum-chemical shifts (lower panels).}
...
\begin{table*}
\caption{Generated structural-model decisions for the reduced-dimensionality
and validation benchmarks. Active variables are counted after symmetry
selection and hard-constraint projection.}
...
model in the symmetry-adapted Cartesian basis gives an aligned Cartesian
root-mean-square displacement (RMSD) of only
$5.2\times 10^{-11}$~\AA{}; robust-loss and multistart checks give RMSDs below
$5\times10^{-4}$~\AA{}, and predicate-width
...
\begin{table*}[t]
\caption{Camphor validation controls relative to the aligned, accepted
no-Kraitchman BDPCS3-predicate geometry.}
...
Figure~\ref{fig:camphor-validation} visualizes the accepted structural shifts
and the principal failure modes.

\begin{figure*}[t]
\centering
\includegraphics[width=0.90\linewidth]{figures/camphor_validation_distributions.pdf}
\caption{Camphor validation stress test. Panels A and B give signed bond-length
and valence-angle shifts for the accepted model; panel C compares the maximum
structural deviations of stability variants.}
...
Figure~\ref{fig:glycine-algorithmic-flow} summarizes the end-to-end
construction.
...
\node[chemobox, right=0.65cm of job] (syn) {Perceived primitives, symmetry,\\descriptors and isotope coverage};
\node[chemobox, below=0.65cm of syn] (adv) {Advisor\\constraints, predicates,\\parameter classes};
\node[autobox, below=0.65cm of job] (gic) {SONIC construction\\non-redundant GICs\\and symmetry labels};
...
\end{tikzpicture}
\caption{End-to-end glycine refinement from a quantum-chemical Cartesian
reference and vibrational corrections to the generated coordinate and
constraint model.}
...
Tables~\ref{tab:glycine-I-qc-rotational} and
\ref{tab:glycine-I-parent-rotational} quantify the improvement from the
quantum-chemical reference to the final semiexperimental geometry.
...
\begin{table*}[t]
\caption{Glycine II constraint-source comparison for the same ten corrected
isotopologue data sets. The last column follows the mean-square convention of
the published refinement.}
...
\begin{table}[t]
\caption{Weighted-predicate tests for the glycine reduced-dimensionality fits.
Predicate widths are $\sigma_R=0.003$~\AA{}, $\sigma_A=0.3^\circ$, and
$\sigma_D=0.5^\circ$.}
...
Table~\ref{tab:glycine-predicate-checks} shows that the predicate layer
improves conditioning without changing either model rank.
...
Figure~\ref{fig:testosterone-scale} and
Table~\ref{tab:testosterone-scale} summarize the molecular scale and the
corresponding rotational-constant residuals.

\begin{figure}[t]
\centering
\includegraphics[width=0.90\linewidth]{figures/testosterone_qc_cropped.jpg}
\caption{Testosterone scale demonstration: a 49-atom fused-ring test of
automated coordinate generation and reduced structural models.}
\label{fig:testosterone-scale}
\end{figure}

\begin{table*}[t]
\caption{Testosterone rotational-constant residuals relative to corrected
experimental equilibrium constants, in MHz.}
\label{tab:testosterone-scale}
\centering
\footnotesize
\begin{tabular}{lrrrr}
\toprule
...
The author thanks Federico Lazzari for independent testing of the companion
implementation used to generate the SONIC coordinate contracts.
\end{acknowledgments}
...
Vincenzo Barone: Conceptualization; Methodology; Software; Validation; Formal
...
article and its supplementary material. The standalone MORPHEUS 0.1.0rc6
source, runnable examples, machine-readable benchmark inputs and outputs,
numerical audit trails, and scripts used for the reported calculations are
publicly available in the revision-pinned repository of Ref.~\citenum{Barone2026MORPHEUSSoftware}.
The companion SONIC methodological preprint is publicly available as
arXiv:2607.16550.\cite{Barone2026SONIC}
