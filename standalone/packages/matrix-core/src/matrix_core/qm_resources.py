"""Backend selection and Basis Set Exchange acquisition for The ONE.
...
from .environment import MatrixEnvironment


QM_SELECTION_SCHEMA = "matrix.the_one.qm_selection.v1"
BASIS_ARTIFACT_SCHEMA = "matrix.the_one.basis_set.v1"
QM_CAPABILITY_REGISTRY_SCHEMA = "matrix.the_one.qm_capability_registry.v1"
...
def backend_capability_records() -> tuple[dict[str, object], ...]:
    """Return the single, serializable capability registry consumed by The ONE."""
...
def qm_capability_registry_schema_path() -> Path:
    return Path(
        str(files("matrix_core").joinpath("schemas/qm-capability-registry-v1.schema.json"))
    )


def qm_selection_schema_path() -> Path:
    return Path(str(files("matrix_core").joinpath("schemas/qm-selection-v1.schema.json")))
...
def _available_locations(environment: MatrixEnvironment) -> dict[str, tuple[str, ...]]:
    ...
    for program in environment.programs:
        if program.category == "qm" and program.enabled and program.available:
            locations.setdefault(program.key, set()).add("local")
    for machine in environment.remote_machines:
        if not machine.enabled:
            continue
        for engine in machine.engines:
            locations.setdefault(engine.casefold(), set()).add(f"remote:{machine.name}")
    gaussian_locations = set(locations.get("gdv", ())) | set(locations.get("g16", ()))
    if gaussian_locations:
        locations["gaussian"] = gaussian_locations
    return {key: tuple(sorted(values)) for key, values in locations.items()}
    ...
    *,
    ...
    if not eligible:
        ...
        reason = (
            ...
            f"The ONE proposes {proposal.method} on {proposal.backend}; accepting a different "
    ...
    if requested != "auto":
        reason = (
            f"The user selected {requested}; The ONE verified the complete "
            f"{method}/{derivative_key} combination ({selected.derivative_support})."
        )
    else:
        reason = (
            f"The ONE selected {selected.backend}: it is configured, supports {method}, "
...
def _fetch_url(url: str) -> bytes:
    request = Request(url, headers={"User-Agent": "MATRIX-The-ONE/1"})
