...
def gicforge_reference_executable(repo_root: Path | None = None) -> Path:
    ...
    build_inputs = (
        [layout.legacy_compile_script, layout.matrix_kernel]
        + sorted(layout.root.rglob("*.f"))
        + sorted(layout.root.rglob("*.inc"))
    )
    executable_mtime = (
        layout.legacy_executable.stat().st_mtime
        if layout.legacy_executable.is_file()
        else float("-inf")
    )
    if any(
...
# Compatibility aliases for external callers and frozen test contracts.
LegacyGICForgeRun = GICForgeReferenceRun
run_legacy_gicforge = run_gicforge_reference
read_legacy_gicforge_run = read_gicforge_reference_run
legacy_gicforge_executable = gicforge_reference_executable
