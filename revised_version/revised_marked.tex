...
\newcommand{\revabstract}[1]{#1}
\emergencystretch=2em
\hypersetup{
  hidelinks,
  pdftitle={Algorithmic Construction and Exploitation of Topology-Constrained Refinement Models for Semiexperimental Equilibrium Structures},
  pdfauthor={Vincenzo Barone},
  pdfsubject={Topology-, symmetry-, and rank-constrained semiexperimental structure refinement},
  pdfkeywords={semiexperimental equilibrium structures, MORPHEUS, generalized internal coordinates, molecular symmetry, rotational spectroscopy}
}
...
\begin{abstract}
\revabstract{Semiexperimental equilibrium structures combine high-resolution rotational spectroscopy with computed vibrational and electronic corrections. \rev{Although the least-squares solution is well established, constructing the inverse model still requires expert choices of coordinates, symmetry, constraints, and parameter classes. We introduce \texttt{MORPHEUS} (Model Optimization and Refinement from Perceived Hierarchy, Equivalence, Uncertainty, and Symmetry), a reproducible framework that makes these choices explicit and tests them against topology, symmetry, rank, isotope coverage, and external reference information. Starting from Cartesian geometry, it builds non-redundant internal coordinates and a refinement model before fitting. Two alternative active spaces are supported: symmetry-adapted generalized internal coordinates and symmetry-adapted Cartesian displacements with translations and rotations removed. Across acyclic, cyclic, planar, fused, bridged, and flexible molecules, the workflow recovers effective dimensions, residuals, structural parameters, and uncertainties without manual construction of the active coordinate model. In camphor, the chemically validated model gives sub-milliangstrom bond corrections and remains stable under basis changes, robust losses, predicate-width scans, and multistarts. A 49-atom testosterone demonstration shows how QC geometries, accelerated vibrational corrections, and class-constrained refinement models can be combined when isotope-rich data are unavailable.}}
...
tests the stability of the selected model. The molecular-state perception is
...
relations for the semiexperimental inverse problem. \rev{They are distinct from
force-field parameters and are used only to express equality, predicate, or
reduced-dimensionality relations within the refinement model.}
...
and finally a large-molecule demonstration.
...
\node[font=\footnotesize\bfseries\sffamily, text=FrameworkGold, align=center, text width=3.2cm] at (7.65,-3.05) {algorithmic\\model generation};
...
PIC set contains \rev{the following coordinate families:} stretches, valence
angles, torsions, out-of-plane deformations,
...
the companion SMITH/SONIC methodological manuscript;\cite{Barone2026SONIC}
...
\caption{Inspectable SONIC variables at the MORPHEUS model boundary.  Three
representative homogeneous families are shown on the reported camphor
geometry: the stored record exposes the primitive expansion and units, the
Cartesian fingerprint makes the associated motion visually auditable, and the
model layer records whether the row is symmetry allowed, linked, constrained,
or protected during rank selection.  Displacement arrows are enlarged for
print; the examples illustrate the traceability contract rather than a finite
refinement step.}
...
\rev{MORPHEUS formulates corrections in the selected GIC space and realizes them in
Cartesian geometry through a rank-controlled minimum-norm or nonlinear
back-transformation. It then evaluates the residuals and updates its
refinement model from the realized geometry. The symmetry-adapted Cartesian route uses
the same selection, constraint projection, and covariance propagation, but
needs no Wilson-B inversion.}
...
coordinate model and, in the symmetry-adapted Cartesian route, the same frozen ORACLE
...
The default \rev{Cartesian} frame places the weighted center at the origin and uses
...
selected model with the alternate active coordinate basis, robust loss
...
\rev{The computational overhead of the MORPHEUS layer is small compared with
the upstream electronic-structure calculations: for the systems considered
here, topology perception, coordinate generation, projection, rank auditing,
and the subsequent refinement are typically completed in seconds on a
laptop-class CPU, including for the 49-atom testosterone case. The sparse
Wilson $\mathbf{B}$ matrix reduces both memory use and the cost of the
associated linear-algebra operations. No additional quantum-chemical
calculation is required by the model-generation step. The cost scales
primarily with the number of atoms, primitive coordinates, isotopologues, and
stability controls, rather than with manual model-design effort.}
...
Testosterone is kept separate and last because it is a large-molecule demonstration
...
validation cases. Testosterone is shown separately in the large-molecule
...
The GIC and symmetry-adapted Cartesian routes both give rank 8 and a rotational RMS
...
\rev{The corresponding auxiliary-relation comparison is given in
Table~\ref{tab:phthalic-predicates}.}
...
\begin{figure*}[!htb]
\centering
\includegraphics[width=0.96\textwidth]{figures/planar_pair_diagnostics.pdf}
...
standard errors are bounded by 3.000~m\AA{} and $0.257^\circ$. \rev{The
corresponding stability distributions are shown in
Figure~\ref{fig:camphor-validation}.}
...
alignment to the selected no-Kraitchman BDPCS3-predicate reference geometry.\par}
\smallskip
\footnotesize
\setlength{\tabcolsep}{4pt}
...
deviation from the BDPCS3 reference. The selected model and predicate-width
...
\rev{The workflow is summarized in Figure~\ref{fig:glycine-algorithmic-flow}.}
...
\rev{The detailed Gly-I rotational diagnostics are reported in Tables~\ref{tab:glycine-I-qc-rotational} and~\ref{tab:glycine-I-parent-rotational}.}
...
condition number decreases from \rev{$1.34\times10^4$} to $8.02\times10^3$, and the
...
effective variables, a condition number of \rev{$2.02\times10^4$}, and a complete
...
parameters. \rev{The comparison of the predicate variants is reported in
Table~\ref{tab:glycine-predicate-checks}.}
...
Model & Rank & Cond. & RMS \\
\midrule
Gly-I constraints & 11 & \rev{$1.34\times10^4$} & 0.0269 \\
Gly-I + predicates & 11 & $8.02\times10^3$ & 0.0389 \\
Gly-II QC constraints & 19 & \rev{$2.02\times10^4$} & 0.2491 \\
...
layer is not limited to these sizes. As a large-molecule demonstration, the framework was
...
The corresponding molecular depiction is shown in Figure~\ref{fig:testosterone-qc},
and the residual comparison is reported in Table~\ref{tab:testosterone-residuals}.

\begin{figure}
\centering
\includegraphics[width=0.90\linewidth]{figures/testosterone_qc_cropped.jpg}
\caption{Testosterone QC geometry used for the large-molecule demonstration.}
\label{fig:testosterone-qc}
\end{figure}

\begin{table}
\caption{Residuals for the testosterone large-molecule demonstration relative
to the corrected experimental equilibrium constants derived from the published
$B_0$ and $\Delta B_\mathrm{vib}$ values.}
\label{tab:testosterone-residuals}
\centering
\begin{tabular}{lrrrr}
\toprule
...
models are applicable, GIC and symmetry-adapted Cartesian refinements give the same
...
The norbornane-derived series adds the \rev{most stringent} validation step. Norcamphor
...
\rev{The framework is more than a convenient input generator.} The final outputs remain
...
\rev{This logic also applies to the large-molecule case.} It does not claim a complete
...
The essential shift is to make model formulation part of the refinement
workflow. The algorithm records chemical judgement as explicit, testable
choices rather than leaving it in hidden manual steps. Although motivated by
...
\rev{The author has no conflicts to disclose.}

\subsection*{Author Contributions}

\rev{Vincenzo Barone: Conceptualization; Data curation; Formal analysis;
...
\rev{The data that support the findings of this study are openly available in
the public MORPHEUS repository on the \texttt{main} branch
Ref.~\citenum{Barone2026MORPHEUSSoftware}.} The repository contains the
MORPHEUS 0.1.0rc8 standalone source, runnable examples, benchmark
inputs and outputs, final-validation data, and the ORACLE-generated primitive
definitions, Wilson $\mathbf{B}$ source, and SMITH-generated SONIC coordinate
contracts used here. The benchmark input/output files, numerical audit trails,
and statistical diagnostics are documented in the Supporting Information.
These files are sufficient to reproduce the reported applications; no separate
methodological package is required. The
final-validation workflow reruns each selected model in
