...
The graph algorithms that generate the local primitive families, remove
within-family redundancies, and form their symmetry-adapted combinations will
be described in a separate methodological paper; here only their operational
role in constrained structure refinement is needed. This boundary is deliberate:
the present paper validates the refinement-model layer, independent of which
topology-perception implementation feeds it. The present
work examines how the generated coordinate spaces are coupled to isotopic
...
\caption{Molecular structures used to test algorithmic model generation. The
acyclic, planar, aromatic, glycine, and anhydride benchmarks are shown as
connectivity diagrams; the bridged norbornane family and testosterone are shown
as three-dimensional structures because their graph topology is not
communicated adequately by a planar drawing. Norbornane is included as the
structural parent of the bridged series, while norcamphor and camphor are the
semiexperimental validation cases. Detailed atom numbering for the tabulated
structural parameters is given in the Supporting Information.}
...
The benchmark input/output files and numerical audit trails are documented in
the Supporting Information. The least-squares refinement engine and the
statistical-diagnostic layer used for the benchmarks reported here are available
from the authors upon request. \texttt{MORPHEUS} is being integrated into the
broader \texttt{MATRIX} framework. The coordinate-generation module remains under
active development for intermolecular systems and other special-coordinate
classes and is planned to be made available with the companion methodological
paper.
