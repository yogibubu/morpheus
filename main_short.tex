...
The implementation used for the present calculations is the semiexperimental
layer of the \texttt{MATRIX} development code base
(\texttt{matrix-morpheus}). It is organized around reusable coordinate and
derivative services. The coordinate-definition service builds the molecular
model from Cartesian atoms and nuclear charges, assigns point-group symmetry
and irreducible representations when requested, and stores the frozen
coordinate definition. A separate B-matrix service evaluates that same
definition for any compatible Cartesian geometry. During refinement, the
engine reuses the stored coordinate model and updates only the derivative
...
systems.\cite{Mendolicchio2025FastDvib} In the \texttt{MATRIX} implementation,
...
graphical audit is given explicitly in the Supporting Information: the
...
the least-squares refinement engine, job/\texttt{xyzin} readers, and
final-validation audit used for the benchmarks reported here is part of the
\texttt{MATRIX} development code base and is available from the authors upon
request. The coordinate-generation module remains under active development for
intermolecular systems and other special-coordinate classes and is planned to
be made available with the companion methodological paper.
